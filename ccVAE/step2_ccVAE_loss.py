...
from .step2_ccVAE_utils import partition
...
def bce(recon_x, x):
    """Computes BCE loss between reconstructed data and ground truth data.
       ...
       MSE loss value
    """
    bce_loss = F.binary_cross_entropy(recon_x, (x > 0).float(), reduction='none')
    return bce_loss
...
def center_loss(embeddings, labels, num_classes=20, feat_dim=10, use_gpu=True):
    ...
    for i, x in enumerate(embeddings):
        scalar_label = torch.tensor(labels[i]).long()
        label = torch.unsqueeze(scalar_label, 0)
        if use_gpu:
            label = label.cuda()

        #distmat = torch.sum((x - centers) ** 2, dim=1)
        ...
        center_loss_list.append(center_loss)

    center_loss_tensor = torch.stack(center_loss_list, dim=0)

    return center_loss_tensor
